...
class ConfigManager:
    ...
    def _validate_config(self):
        ...
        """
        required_sections = ['MONGO_DB', 'REST', 'TRANSLATE', 'TEXT', 'CACHE', 'TTS', 'DEVICE', 'STT']
        ...
        self._validate_section_keys('MONGO_DB', [
            'CONNECTION_STRING', 'MONGO_DATABASE', 'MONGO_USERS_COLLECTION',
            'MONGO_USER_FILES_COLLECTION', 'MONGO_TTS_FILES_COLLECTION'
        ])
        self._validate_section_keys('REST', [
            'ALLOWED_EXTENSIONS', 'HOST', 'MAX_CONTENT_LENGTH_MB',
            'MAX_TOTAL_SIZE_GB', 'PORT', 'UPLOAD_FOLDER'
        ])
        self._validate_section_keys('TEXT', ['MAX_TOKEN'])
        self._validate_section_keys('TRANSLATE', ['AVAILABLE_MODELS'])
        self._validate_section_keys('TTS', [
            'AVAILABLE_MODELS', 'AVAILABLE_SPEAKERS', 'AVAILABLE_LANGUAGES',
            'DEFAULT_OUTPUT_FILENAME', 'DEFAULT_SAMPLERATE', 'DEFAULT_MIMETYPE', 'DEFAULT_AS_ATTACHMENT'
        ])
        self._validate_section_keys('STT', ['MODEL'])
    ...
    def get_private_key_path(self) -> str:
        """
        Retrieves the path to the private keys file from the configuration.
        """
    ...
    def get_tts_speakers(self) -> list:
        """
        Returns a sorted list of available TTS speakers.
        """
    ...
    def get_stt_models(self) -> str:
        """
        Retrieves the speech-to-text model configuration from the STT section.

        Returns:
            str: The STT model name (default is "turbo").
        """
        return self.get_config_value('STT', 'MODEL', str, default="turbo")