...
class TestMongoDBManager(unittest.TestCase):
    ...
    def test_get_collection_nonexistent(self):
        """
        Verify that get_collection creates a new collection if it does not exist.
        """
        ...
        self.assertEqual(collection, self.mock_db.__getitem__.return_value)
    ...
    def test_update_document_existing(self):
        """
        Verify that update_document updates an existing document.
        """
        collection_name = "test_collection"
        query = {"key": "value"}
        update_data = {"$set": {"new_key": "new_value"}}
        mock_collection = MagicMock()
        ...
        result = self.manager.update_document(collection_name, query, update_data, upsert=False)

        mock_collection.update_one.assert_called_once_with(query, update_data, upsert=False)
        self.assertEqual(result.matched_count, 1)
        self.assertEqual(result.modified_count, 1)

    def test_update_document_upsert(self):
        """
        Verify that update_document inserts a document if it does not exist when upsert=True.
        """
        collection_name = "test_collection"
        query = {"key": "value"}
        update_data = {"$set": {"new_key": "new_value"}}
        ...
        result = self.manager.update_document(collection_name, query, update_data, upsert=True)

        mock_collection.update_one.assert_called_once_with(query, update_data, upsert=True)
